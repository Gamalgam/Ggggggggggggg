...
class Game(object):
    ...
    def replace_formode(self):
        if self.color_mode == "white":
            if self.mode == None or self.mode in LIST_CLASSIC:
                ...
                print(
                    f"الوضع '{self.mode}' غير موجود. تم استبداله بالوضع 'الكلاسيكي'.")
                self.deck._fill_classic_(game_color=self.color_mode)
    ...
    def turn(self):
        ...
        self.logger.debug("اللاعب التالي")
    ...
    def play_card(self, card):
        ...
        self.logger.info("يتم لعب البطاقة " + repr(card))
        if card.value == c.SKIP:
            self.turn()
        elif card.special == c.DRAW_FOUR:
            self.draw_counter += 4
            self.logger.debug("عداد السحب زاد بمقدار 4")
        elif card.value == c.DRAW_TWO:
            self.draw_counter += 2
            self.logger.debug("عداد السحب زاد بمقدار 2")
