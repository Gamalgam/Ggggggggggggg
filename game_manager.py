...
class GameManager(object):
    ...
    def new_game(self, chat):
        ...
        self.logger.debug("إنشاء لعبة جديدة في الدردشة :" + str(chat.title) + " id :" + str(chat_id) + " (@" + str(chat.username) + ")")
        game = Game(chat)
    ...
    def join_game(self, user, chat):
        ...
        self.logger.info("الدخول إلى اللعبة :" + " \"" + str(chat.title) + "\" (@" + str(chat.username) + ") المستخدم : @" + str(user.username))
        ...
        if game.started:
            if game.mode in SETS_MODES:
                ...
                raise NotEnoughGameMode("تعذر العثور على الوضع المحدد")
    ...
    def end_game(self, chat, user):
        """End a game"""
        self.logger.info("انتهت اللعبة في الدردشة " + str(chat.id) + " @" + str(chat.username))
