...
class Player(object):
    ...
    def playable_cards(self):
        ...
        self.logger.debug("آخر بطاقة كانت " + str(last))
        ...
        if self.game.color_mode == "white":
            if not mode:
                if (card.color != last.color and card.value != last.value and
                    ...
                    self.logger.debug("لا يمكن لعب colorchooser على الآخر")
                    is_playable = False
                elif not last.color:
                    self.logger.debug("آخر بطاقة لا تحتوي على لون")
                    is_playable = False
            else:
