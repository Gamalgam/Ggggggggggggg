...
send_async(bot, chat.id,
           text=__("تم تقليص وقت انتظار اللاعب "
                   "إلى {time} ثانية.\n"
                   "الدور التالي للاعب: {name}", multi=game.translate)
           .format(time=n,
                   name=display_name(next_player.user))
           )
logger.info("{player} تم تخطيه! "
...
if job_queue:
    ...
    try:
        ...
        send_async(bot, chat.id,
                   text=__("انتهى وقت {name1}"
                           "وتم حذفه من اللعبة!\n"
                           "الدور التالي للاعب: {name2}", multi=game.translate)
        ...
        send_async(bot, chat.id,
                   text=__("انتهى وقت {name}!"
                           "وتم حذفه من اللعبة!\n"
                           "اللعبة انتهت.", multi=game.translate)
...
def do_play_card(bot, player, result_id):
    """لعب البطاقة المحددة وإرسال تحديث إلى المجموعة إذا لزم الأمر"""
    game = player.game
    chat = game.chat
    user = player.user
    skipped_player = game.current_player
    ...
    if game.choosing_color:
        send_async(bot, chat.id, text=__(
            "يرجى اختيار اللون", multi=game.translate))

    if len(player.cards) == 1:
        send_async(bot, chat.id, text="أونو!")

    if len(player.cards) == 0:
        send_async(bot, chat.id,
                   text=__("{name} فاز!", multi=game.translate)
        ...
        try:
            ...
            send_async(bot, chat.id,
                       text=__("اللعبة انتهت!", multi=game.translate))
...
def do_draw(bot, player):
    ...
    try:
        ...
        send_async(bot, player.game.chat.id,
                   text=__("في الكومة لم تبقَ بطاقات.",
...
def do_call_bluff(bot, player):
    ...
    if player.prev.bluffing:
        send_async(bot, chat.id,
                   text=__("نعم. هذه خدعة! {name} يأخذ 4 بطاقات",
                           multi=game.translate)
        ...
        game.draw_counter += 2
        send_async(bot, chat.id,
                   text=__("{name1} لم يكن يخدع! {name2} يأخذ 6 بطاقات.",
                           multi=game.translate)
...
def start_player_countdown(bot, game, job_queue):
    ...
    if game.mode == 'fast':
        ...
        logger.info("تم تقليص وقت {player} إلى {time} ثواني."
